...
const FeedDetails: FC = () => {
    ...
    const orders  = useSelector(
        (store) => (path.indexOf('feed') === -1) ? store.wsConnectUser.data.orders : store.wsConnect.data.orders
    );
    const loadingOver = useSelector(
        (store) => (path.indexOf('feed') === -1) ? store.wsConnectUser.wsConnected : store.wsConnect.wsConnected
    );
